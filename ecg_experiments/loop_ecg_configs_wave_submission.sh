...
for d in $(ls -d ./wave_rate_prime_step_configs_1s-deadline_50-item-window/*); do
    # deploy all time series files in each folder
    ...
    for f in $(cd $d ; ls * ); do
        ...
        time ./time_series_query tmp_time_series_config_deployment.json log_config.yml > $output_file_name 2>&1
        if [ $? -eq 0 ]; then
            rm $output_file_name
        fi
