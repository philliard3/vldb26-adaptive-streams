#!/bin/bash
# Deploy Exp2 Revised (Feb 19)
# Usage: ./deploy_exp2_revised.sh [arguments for make_exp2_revised_configs.py]
# Example: ./deploy_exp2_revised.sh --mode full --revision both --strat greedy

set -e

DEPLOYMENT_BASE="deployment_exp2"
...
# 1. Generate Configs (and transfer manifest)
echo "Generating Exp2 Revised Configs with args: ${CONFIG_ARGS[*]}"
# Clean prior generated artifacts so only requested config matrix is produced.
rm -rf "$DEPLOYMENT_BASE/configs" "$DEPLOYMENT_BASE/queries"
rm -f exp2_revised_transfers.txt
python3 make_exp2_revised_configs.py "${CONFIG_ARGS[@]}"

# 2. Server Setup (RUNTIME_DIR)
RUNTIME_DIR="~/animal_query_artifacts"

# Read DNS from current_vm.txt
...
# 4. Upload Configs & Queries
echo "Uploading configs & queries..."
ssh -i "$KEY" "$SERVER" "mkdir -p $RUNTIME_DIR/deployment_exp2/configs $RUNTIME_DIR/deployment_exp2/queries"
# Ensure removed local configs/queries are also removed remotely to avoid stale runs.
rsync -avz --delete -e "ssh -i $KEY" ./deployment_exp2/ "$SERVER:$RUNTIME_DIR/deployment_exp2/"

# 5. Upload Pre-classifiers (Dynamic)
echo "Uploading pre-classifiers..."
ssh -i "$KEY" "$SERVER" "mkdir -p $RUNTIME_DIR/exp2_preclassifiers"

if [ -f "exp2_revised_transfers.txt" ]; then
    count=0
    while IFS="|" read -r local server; do
        clean_server=${server#./}
        # Only echo every 10th file to reduce noise if many
        count=$((count + 1))
        if (( count % 5 == 0 )); then
    ...
    done < exp2_revised_transfers.txt
    echo "  Uploaded $count pre-classifiers."
else
    echo "WARNING: exp2_revised_transfers.txt not found. No pre-classifiers uploaded."
fi

# 6. Upload Runner
echo "Uploading runner..."
rsync -avz -e "ssh -i $KEY" run_exp2_revised.sh "$SERVER:$RUNTIME_DIR/"
rsync -avz -e "ssh -i $KEY" animal_query/log_config.yml "$SERVER:$RUNTIME_DIR/"

ssh -i "$KEY" "$SERVER" "chmod +x $RUNTIME_DIR/run_exp2_revised.sh"

# 7. Optional: Sync dataset metadata/images once
...
# 8. Optional dataset preflight check (if metadata path was provided for sync)
...
echo "  cd $RUNTIME_DIR"
echo "  nohup ./run_exp2_revised.sh > exp2_revised.log 2>&1 &"
echo "=========================================="
